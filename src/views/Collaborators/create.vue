    ...
    <div class="p-2">
        <div class="bg-white shadow-sm rounded-lg overflow-hidden">
            <div class="p-4 text-center bg-primary text-shadow">
                <h1 class="text-light display-4 mb-0">Create a new collaborator</h1>
                <h6 class="text-warning lead font-weight-normal">Fill out the form below</h6>
            </div>
            <form autocomplete="off" class="p-4" @submit.prevent="onSubmit()">
                ...
                <div class="grid-container mb-4">
                    ...
                    <div class="grid">
                        ...
                        <input class="form-control" type="text" placeholder="johndoe" name="username" v-model="mainForm.username" :class="{ 'is-invalid': mainForm.errors.has('username') }">
                    ...
                    <div class="grid">
                        ...
                        <input class="form-control" type="text" name="password" v-model="mainForm.password" :class="{ 'is-invalid': mainForm.errors.has('password') }">
...
export default {
    components: {
        Table: () => {
            return import('@/components/collaborator/Table');
        },
    },
    data() {
        return {
    ...
    methods: {
        onSubmit() {
            this.mainForm.post('/collaborators/create').then(response => {
                let collaborator_id = response.collaborator_id;
                
                this.$refs.leavesSection.submit(collaborator_id);
                this.$refs.skillsSection.submit(collaborator_id);
                this.$refs.trainingsSection.submit(collaborator_id);
                this.$refs.evaluationsSection.submit(collaborator_id);
            }).catch(error => {
                console.log(error);
            })
        }
